...
function wpgmza_nvc_register_click_checker(map_id, marker_id, current_marker){
	...
	google.maps.event.addListener(current_marker, "click", function(event){
...
function wpgmza_nvc_open_web(map_id, marker_id, current_marker){
	var min_lat = current_marker.position.lat() - wpgmza_nvc_web_range;
    var max_lat = current_marker.position.lat() + wpgmza_nvc_web_range;
    var min_lng = current_marker.position.lng() - wpgmza_nvc_web_range;
    var max_lng = current_marker.position.lng() + wpgmza_nvc_web_range;
    ...
    if(typeof marker_array !== "undefined" && typeof marker_array[map_id] !== "undefined"){
	    ...
	    for(var i in marker_array[map_id]){
	       	if(parseInt(i) !== parseInt(marker_id)){
		        var current_lat = marker_array[map_id][i].position.lat();
		        var current_lng = marker_array[map_id][i].position.lng();
		        if(current_lat > min_lat && current_lat < max_lat){
		          	if(current_lng > min_lng && current_lng < max_lng){
		                ...
		                var newLatlng = wpgmza_nvc_get_point_in_circle(wpgmza_nvc_marker_status[map_id][marker_id]['original_pos'], starting_degree);
		                 
		                starting_degree += wpgmza_nvc_web_degree_increment;
		                marker_array[map_id][i].setPosition(newLatlng);
		                 
		                var pathFromClick = [
		                  {lat: originalLatLng.lat(), lng: originalLatLng.lng()},
...
function wpgmza_nvc_get_point_in_circle(center, dg){
  	...
  	var lat = Math.sin(dg * Math.PI / 180) * r + center.lat();
  	var lng = Math.cos(dg * Math.PI / 180) * r + center.lng();

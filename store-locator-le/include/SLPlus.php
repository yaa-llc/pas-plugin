...
if ( ! class_exists( 'SLPlus' ) ) {
	...
	class SLPlus extends SLP_Object_With_Objects {
		...
		function activate_or_update_slplus() {
			if ( version_compare( $this->installed_version , SLPLUS_VERSION , '<' ) ) {
				...
				if ( $this->Activation->disabled_experience ) {
					add_action( 'admin_notices' , create_function( '' , "echo '<div class=\"error\"><p>" . __( 'You must upgrade Experience add-on to 4.7.6 or higher or your site will crash. ' , 'store-locator-le' ) . "</p></div>';" ) );
				}
			}
		}
		...
		function createobject_AJAX() {
			if ( ! isset( $this->ajax ) ) {
				$this->AjaxHandler = $this->AJAX = $this->ajax = new SLP_AJAX();
			}
		}
		...
		public function initialize_after_plugins_loaded() {
			...
			// Debug?
			if ( $this->SmartOptions->enable_wp_debug->is_true ) {
				$this->debug_mode();
			}
		...
		private function get_google_maps_url() {
			...
			//
			$client_id = ! empty( $this->options_nojs[ 'google_client_id' ] ) ? '&client=' . $this->options_nojs[ 'google_client_id' ] . '&v=3' : '';

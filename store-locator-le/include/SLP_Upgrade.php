...
if ( ! class_exists('SLP_Upgrade') ) {
	...
	class SLP_Upgrade extends SLPlus_BaseClass_Object {
		 ...
		 *
		 * @uses \SLP_Upgrade::replace_multiple_backslashes_with_one
		 */
		private function fix_backslashes() {
			if ( ! version_compare( $this->slplus->installed_version , '4.8.8' , '<' ) ) return;
		 ...
		 *
		 * @used-by \SLP_Upgrade::fix_backslashes
		 *
		 * @param $value
		 *
		 * @param $key

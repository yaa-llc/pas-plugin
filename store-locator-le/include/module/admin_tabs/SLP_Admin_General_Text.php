	...
	class SLP_Admin_General_Text {
		...
		private function set_text_strings() {
			...
			if ( ! $slplus->AddOns->get( 'slp-premier' , 'active' ) ) {
				$this->text[ 'description' ][ 'url_control_description' ] .= ' ' . sprintf( __( '%s provides additional URL controls. ' , 'store-locator-le' ), $slplus->AddOns->get_product_url( 'slp-premier' ) );
			}

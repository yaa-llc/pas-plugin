...
if ( ! class_exists( 'SLP_BaseClass_Admin' ) ) {
    ...
    class SLP_BaseClass_Admin extends SLP_Object_With_Objects {
	        ...
	        if ( ! empty ( $files ) ) {
		        foreach ( $files as $file ) {
			        if ( file_exists( $this->addon->dir . $file ) ) {
				        ...
				        wp_enqueue_script( $handle , $this->addon->url . '/' . $file , $this->js_requirements ,false , true );
				        break;
			        }
		        }
	        }
        ...
        protected function ok_to_enqueue_admin_js( $hook ) {
        	if ( in_array( $hook, $this->slplus->AdminUI->cache[ 'admin_pages' ] ) ) return true;

			if ( strpos( $hook , SLP_ADMIN_PAGEPRE ) === 0 ) {
				$this->slplus->AdminUI->cache[ 'admin_pages' ][] = $hook;
				return true;
			}

	        $this->slplus->AdminUI->cache[ 'admin_pages' ] = apply_filters( 'slp_managed_admin_pages' , $this->slplus->AdminUI->cache[ 'admin_pages' ] );

			return ( in_array( $hook , $this->slplus->AdminUI->cache[ 'admin_pages' ] ) );
        }

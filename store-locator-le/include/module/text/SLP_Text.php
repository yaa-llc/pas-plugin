...
class SLP_Text extends SLP_Object_With_Objects {
	...
	public function get_text_string( $slug , $log_no_default = true ) {
		...
		if ( ! isset ( $this->text_strings[ $slug[0] ] ) || ! isset( $this->text_strings[ $slug[0] ][ $slug[1] ] ) ) {
			...
			$this->text_strings[ $slug[0] ][ $slug[1] ] = ( $slug[0] !== 'general' ) ? $this->get_text_string( $slug[1] , false ) : $slug[1];
		} else {
	...
	private function init_strings( $slug ) {
		...
		switch ( $area_slug ) {
			case 'general':
				...
				$this->text_strings[ $area_slug ][ 'slp_settings'   ]               = __( 'Store Locator Plus Settings' , 'store-locator-le' );
			...
			case 'messages':
				$this->text_strings[ $area_slug ]= array(
					'no_api_key'            => sprintf( __( 'This site most likely needs a <a href="%s" target="store_locator_plus">%s</a> key. ', 'store-locator-le' ),
										        ...
										        '<br/>' .
										        __( 'Do not set restrictions on the API key.' , 'store-locator-le' ) .
										        '<br/>' .
										        __( 'Save it under Store Locator Plus | General | Server | Google Developers Console in the Google Browser Key setting.' , 'store-locator-le' ),

					'script_not_loaded'     => __( 'The Store Locator Plus script did not loaded properly.' , 'store-locator-le' ) ,
					'slp_system_message'    => __( 'Store Locator Plus System Message' , 'store-locator-le' ),
				);
				break;

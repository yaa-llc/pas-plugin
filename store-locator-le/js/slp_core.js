...
var slp_LocationServices = function() {
   ...
   *
   * When setting currentLocation the callback and errorCallback functions must
   * be defined. See the sensor.currentLocation call down below for the
   * return-to place for these two functions passed as variables (down around
   * line 1350).
   *
   * @param callback
   * @param errorCallback
 ...
 * @param string            markerImage todo: load a custom icon, null for
 *   default
 ...
 * @property  {google.maps.LatLng}      homePoint           - The user-entered
 *   home point.
 * @property  {google.maps.InfoWindow}  infoWindow
 *
 * @property  {object}                  options
 * @property  {google.maps.LatLng}|null      options.center      - The map
 *   center point.
 */
var slp_Map = function() {
  ...
  this.build_map = function( center ) {
    ...
    if ( this.gmap === null ) {
      ...
      // TODO: SLP 5.0 Remove this when legacy Enhanced Maps support is no
      // longer needed/desired.
      if ( typeof slplus.options.map_options_scrollwheel !== 'undefined' ) {
        this.options[ 'scrollwheel' ] = slplus.options.map_options_scrollwheel;
      }
      if ( typeof slplus.options.map_options_mapTypeControl !== 'undefined' ) {
        this.options[ 'mapTypeControl' ] = slplus.options.map_options_mapTypeControl;
      }
       ...
       * @filter  map_options
       *
       * @param   object   this.options   The current map options for
       *   google.maps.Map
  ...
  /**
   * Notifies as the map changes that we'd like to be nofified when the tiles
  ...
  this.putMarkers = function( markerListNatural ) {
    ...
    if ( markerCount === 0 ) {
      ...
      document.getElementById(
        'map_sidebar' ).innerHTML = '<div class="no_results_found"><h2>' +
        slplus.options.message_no_results + '</h2></div>';
      ...
      for ( var markerNumber = 0; markerNumber < markerCount; ++markerNumber ) {
        ...
        locationIcon = _this.set_location_icon(
          markerListNatural[ markerNumber ].icon );

        this.markers.push(
          new slp_Marker( this, markerListNatural[ markerNumber ].data.sl_store,
        ...
        //
        if ( sidebar ) {
          jQuery( results_html.insert_rows_at ).
            append( this.createSidebar( markerListNatural[ markerNumber ] ) );
          ...
          jQuery( '#slp_results_wrapper_' +
            markerListNatural[ markerNumber ].id ).
            on(
              'click',
              null,
              {
  ...
  this.set_location_icon = function( icon ) {
    ...
    (icon.length > 4) ? icon : slplus.options.map_end_icon );
  };
  ...
  this.getSearchAddress = function( defaultAddress ) {
    ...
    if ( !searchAddress ) {
      if ( (slplus.options.use_sensor != '0') && (sensor.lat !== 0.00) &&
        (sensor.lng !== 0.00) ) {
  ...
  this.process_ajax_response = function( response ) {
    ...
    if ( valid_response ) {
       ...
       *
       * @param   object   response.response   a list of map markers in an
       *   ordinal array (pass by reference)
       *
  ...
  this.createSidebar = function( aMarker ) {
    ...
    aMarker.search_address = encodeURIComponent(
  ...
  jQuery.fn.extend( {
    ...
    formParams: function( convert ) {
      if ( this[ 0 ].nodeName.toLowerCase() == 'form' && this[ 0 ].elements ) {
        ...
        return jQuery( jQuery.makeArray( this[ 0 ].elements ) ).
          getParams( convert );
      }
      return jQuery( 'input[name], textarea[name], select[name]', this[ 0 ] ).
        getParams( convert );
    },
...
slp.setup_helpers = function() {
    ...
    //
    slp.important_objects = {
        'address_input' : jQuery( '#addressInput'  ) ,
        'find_button'   : jQuery( '#addressSubmit' )
    };
  ...
  String.prototype.replace_shortcodes = function() {
    ...
    return this.replace(
      ...
      function(
        match, shortcode, attribute, modifier, modarg, modifier2, modarg2 ) {
    ...
    if ( sensor.LocationSupport ) {
      sensor.currentLocation(
        ...
        function( loc ) {
          ...
          cslmap.build_map(
            new google.maps.LatLng( loc.coords.latitude,
...
slp.subscriptions = ( function() {
  var loading_class = 'loading_locations';
    ...
    this.show_locations_processing = function() {
        slp.important_objects.find_button.addClass( loading_class );
        slp.important_objects.address_input.addClass( loading_class );
    };
    ...
    this.remove_locations_processing = function() {
        slp.important_objects.find_button.removeClass( loading_class );
        slp.important_objects.address_input.removeClass( loading_class );
    };
...
slp.start_map = function() {
  if ( jQuery( '#map' ).length ) {
    if ( typeof slplus !== 'undefined' ) {
      ...
      jQuery( '#map' ).
        html( 'Store Locator Plus did not initialize properly.' );
    }
  }
};
